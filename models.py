...
from tensorflow.keras.layers import Input, Flatten, Dense, Conv2D, MaxPooling2D, Concatenate, ZeroPadding2D
...
class BaseModel():
    ...
    def save_training_history(self, path):
        if self.trained:
            ...
            sns.lineplot(self.history.epoch,
                         self.history.history['accuracy'], label='train')
            try:
                sns.lineplot(self.history.epoch,
                             self.history.history['val_accuracy'], label='valid')
            except KeyError:
                pass
            ...
            try:
                sns.lineplot(self.history.epoch,
                             self.history.history['val_loss'], label='valid')
            except KeyError:
                pass
            pyplot.title('Loss')
            pyplot.tight_layout()
...
def cnn_for_raw_img(in_shape, out_shape):
    ...
    maxpool2d_1 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_1')(batchnorm_2)
    ...
    maxpool2d_2 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_2')(batchnorm_4)
    ...
    maxpool2d_3 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_3')(batchnorm_6)
...
class CNNModel(BaseModel):
    ...
    def model_builder(self, in_shape, out_shape):
        ...
        model_out = Dense(out_shape, activation="softmax",
                          name="out_layer")(cnn_out)
...
def cnn_for_roi1_img(in_shape):
    ...
    maxpool2d_1 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_1_roi1')(batchnorm_2)
    ...
    maxpool2d_2 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_2_roi1')(batchnorm_4)
...
def cnn_for_roi2_img(in_shape):
    ...
    maxpool2d_1 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_1_roi2')(batchnorm_2)
    ...
    maxpool2d_2 = MaxPooling2D(pool_size=(
        2, 2), name='maxpool2d_2_roi2')(batchnorm_4)
...
class CNN_ROI1_ROI2Model(BaseModel):
    ...
    def model_builder(self, in_shape, out_shape):
        rawimg_in, rawimg_out = cnn_for_raw_img(
        ...
        out = Dense(out_shape, activation="softmax",
                    name="out_layer")(dropout_2)
    ...
    def train(self, Xtrain_img, Xtrain_roi1, Xtrain_roi2, y_train, validation_data, batch_size=24, epochs=50,
        ...
        if train_datagen is None:
            ...
            self.history = self.model.fit(
                train_datagen(Xtrain_img, Xtrain_roi1, Xtrain_roi2,
...
class CNN_ROI1_ROI2_HOGFeat_Model(BaseModel):
    ...
    def model_builder(self, in_shape, out_shape):
        ...
        out = Dense(out_shape, activation="softmax",
                    name="out_layer")(dropout_3)
        self.model = Model(inputs=models_in, outputs=out,
                           name="CNN_ROI1_ROI2_HOGFEAT")
    ...
    def train(self, Xtrain_img, Xtrain_roi1, Xtrain_roi2, Xtrain_hogfeat, y_train, validation_data, batch_size=24, epochs=50,
        ...
        if train_datagen is None:
            ...
            self.history = self.model.fit(
                train_datagen(Xtrain_img, Xtrain_roi1, Xtrain_roi2,
                              Xtrain_hogfeat, y_train, batch_size=batch_size),
                validation_data=validation_data,
                steps_per_epoch=steps_per_epoch,
                epochs=epochs,
                callbacks=callbacks,
...
class CNN_ROI1_ROI2_KLDIST_Model(BaseModel):
    ...
    def model_builder(self, in_shape, out_shape):
        ...
        roi1_in, roi1_out = cnn_for_roi1_img(in_shape=(25, 50, 1))
        roi2_in, roi2_out = cnn_for_roi2_img(in_shape=(25, 50, 1))
        kldist_in, kldist_out = nn_for_keylandmarks_distance(
            input_shape=(77, 1))
    ...
    def train(self, Xtrain_img, Xtrain_roi1, Xtrain_roi2, Xtrain_kldist, y_train, validation_data, batch_size=24, epochs=50,
        ...
        if train_datagen is None:
            ...
            self.history = self.model.fit(
                train_datagen(Xtrain_img, Xtrain_roi1, Xtrain_roi2,
                              Xtrain_kldist, y_train, batch_size=batch_size),
                validation_data=validation_data,
                steps_per_epoch=steps_per_epoch,
                epochs=epochs,
                callbacks=callbacks,
...
def alexnet_model(img_shape, n_classes, l2_reg=0., weights=None):

    # Initialize model
    alexnet = Sequential()

    # Layer 1
    alexnet.add(Conv2D(64, (3, 3), input_shape=img_shape,
...
def VGG16_model(img_shape, n_classes, weights_path=None):
...
class VGG16(BaseModel):
    ...
    def model_builder(self, in_shape, n_classes):
        self.model = VGG16_model(in_shape, n_classes)
...
class AlexNet(BaseModel):
    ...
    def model_builder(self, in_shape, n_classes):
        self.model = alexnet_model(in_shape, n_classes)
